...
if ($argc < 1) {
    printf STDERR "/gff3togtf.pl - transform SGD gff3 format to UCSC gtf\n";
    printf STDERR "usage: ./gff3togtf.pl <file.gff3>\n";
    printf STDERR "\te.g.: gff3togtf.pl S.cerevisiae.gff3\n";
...
while (my $line = <FH>) {
    ...
    next if ($fields[2] eq "chromosome" || $fields[2] eq "exon");
    if (scalar(@fields) != 9) {
	...
	my $note = "";
	if (exists($field9Hash{'DESCRIPTION'})) {
	    $note = $field9Hash{'DESCRIPTION'};
	}
	my $decoded = "";
	if (length($note) > 0) {
	    $decoded = decode($note);
