...
sub getDbFromPath {
  ...
  if (! defined($db)) {
    ...
    if ($val =~ m#^/home/k/kunertova5/data/genomes/#) {
	$val =~ s#^/home/k/kunertova5/data/genomes/##;
	$val =~ s#/.*##;
	$db = $val;
...
sequence is not performed here, but later on by blastz cluster jobs.";
  my $bossScript = newBash HgLocaleScript("$runDir/doPartition.bash", $paraHub,
...
sub doBlastzClusterRun {
  ...
  HgAutomate::verbose(1,
	"DOBLASTZCLUSTERRUN ".$outRoot."\n");
  &HgAutomate::mustMkdir($outRoot);

  my $tpartsDir = "$runDir/tParts";
  opendir(DH, $tpartsDir);
  my @files = readdir(DH);
  closedir(DH);
  foreach my $file (@files) {
    if ($file =~ /^(part[0-9]+\.lst)/) {
        &HgAutomate::mustMkdir("$outRoot/$file");
    }
  }
...
_EOF_
  ...
  $bossScript->execute();
  $whatItDoes = "It sets up and performs the big cluster blastz run.";
  $bossScript = newBash HgLocaleScript("$runDir/doClusterRun1.sh", $paraHub,
				      $runDir, $whatItDoes, $DEF);
...
#	"stage, or move aside/remove $runDir/ and run again.\n";
  #} elsif ((-e "$runDir/gsub" || -e "$runDir/jobList") && ! $opt_debug) {
  if ((-e "$runDir/gsub" || -e "$runDir/jobList") && ! $opt_debug) {
  ...
  #HgAutomate::verbose(1,"Opt_debug\n".$opt_debug."\n");
...
each subdirectory of $outRoot into a per-target-chunk file.";
  my $bossScript = newBash HgLocaleScript("$runDir/doCatRun.sh", $paraHub,
				      $runDir, $whatItDoes, $DEF);
  my $gensub2 = '/kentsrc/kent/src/parasol/bin/gensub2';
  $bossScript->add(<<_EOF_
...
_EOF_
  ...
  $bossScript->execute();
  $whatItDoes = "It sets up and performs the big cluster blastz run.";
  $bossScript = newBash HgLocaleScript("$runDir/doCatRun1.sh", $paraHub,
				      $runDir, $whatItDoes, $DEF);
...
sub doChainRun {
  ...
  &HgAutomate::mustMkdir($runDir);
  &HgAutomate::makeGsub($runDir,
	       "./chain.sh \$(file1) {check out line+ chain/\$(file1).chain}");
  ...
  my $fh = &HgAutomate::mustOpen(">$runDir/chain.sh");
  print $fh  <<_EOF_
#!/bin/sh -ef
...
to each target sequence.";
  my $gensub2 = '/kentsrc/kent/src/parasol/bin/gensub2';
  my $bossScript = newBash HgLocaleScript("$runDir/doChainRun.sh", $paraHub,
				      $runDir, $whatItDoes, $DEF);
  $bossScript->add(<<_EOF_
chmod a+x chain.sh
$gensub2 pslParts.lst single gsub jobList
mkdir chain liftedChain
sed -re "s/(\\.\\/chain\\.sh\\spart[0-9]+\\.lst\\s)\\{check\\sout\\sline\\+\\s([a-zA-Z0-9\\.\\/]+)\\}/\\1\\2/g" jobList > jobList1
_EOF_
  );
  #rmdir liftedChain
  $bossScript->execute();
  $bossScript = newBash HgLocaleScript("$runDir/doChainRun1.sh", $paraHub,
				      $runDir, $whatItDoes, $DEF);
...
sub postProcessChains {
  ...
  my $cmd = "find $runDir/run/chain -name \"*.chain\" ";
  $cmd .= "| chainMergeSort -inputList=stdin ";
  $cmd .= "| gzip -c > $runDir/$chain";
  &HgAutomate::run($cmd);
  if ($splitRef) {
    &HgAutomate::run("chainSplit $runDir/chain $runDir/$chain");
  }
...
and loads the net table.";
  my $bossScript = newBash HgLocaleScript("$runDir/loadUp.sh", $dbHost,
				      $runDir, $whatItDoes, $DEF);
  $bossScript->add(<<_EOF_
# Load chains:
_EOF_
...
sub makeDownloads {
  ...
  if (-e "$runDir/$tDb.$qDb.net") {
    &HgAutomate::run("gzip $runDir/$tDb.$qDb.net");
  }
...
over.chain file in the liftOver dir.";
  my $bossScript = newBash HgLocaleScript("$runDir/makeMd5sum.sh", $workhorse,
				      $runDir, $whatItDoes, $DEF);
  ...
  my $liftOverDir = "$buildDir/../data/$tDb/liftOver";
  $bossScript->add(<<_EOF_
mkdir -p $liftOverDir
...
sub installDownloads {
  ...
  my $bossScript = newBash HgLocaleScript("$runDir/installDownloads.sh", $dbHost,
				      $runDir, $whatItDoes, $DEF);
  $bossScript->add(<<_EOF_
...
already been performed).";
  my $bossScript = newBash HgLocaleScript("$buildDir/cleanUp.sh", "locale",
				      $runDir, $whatItDoes, $DEF);
  $bossScript->add(<<_EOF_
rm -fr $outRoot/
$rootCanal
...
/bin/sh -c "rm -fr $buildDir/run.blastz/tParts/*.2bit || true"
/bin/sh -c "rm -fr $buildDir/run.blastz/qParts/*.2bit || true"
